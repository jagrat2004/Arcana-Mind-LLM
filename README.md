# 🔮 Interactive Tarot Reading Application
...
│   └── readme/
│       ├── TheMagicianAI.gif      # Project asset
...
### Issue: Slow performance or LLM timeouts

**Solutions:**
- Increase available RAM to your system
- Reduce temperature from 0.9 to 0.7 for faster responses
- Use a smaller model: `ollama pull phi2` (smaller than phi3:mini)
- Close other applications consuming resources

### Issue: Images not displaying

**Check:**
1. Image files exist in `images/` directory
2. Filenames in CSV match image filenames exactly
3. File extensions are `.jpg` (lowercase)

### Issue: CSV data not loading

**Check:**
```python
# Verify separator (semicolon) and encoding (latin1)
df = pd.read_csv(csv_file_path, sep=';', encoding='latin1')
```

Changes needed:
- Modify `sep=` parameter if using different delimiter
- Modify `encoding=` if using different character encoding
...
### Example: Career Decision Reading
...
**Step 2: Settings**
- Select: **5 cards**
- Question: **"I've been offered a promotion but it requires relocating. Should I accept?"**

**Step 3: Cards Drawn** (Example)
- Card 1: The Chariot (Upright) - Control, determination, success
- Card 2: The Lover's (Reversed) - Disharmony, misalignment of values
- Card 3: The Empress (Upright) - Abundance, nurturing, creativity
- Card 4: The Tower (Reversed) - Avoidance of disaster, resisting change
- Card 5: The Star (Upright) - Hope, purpose, inspiration

**Step 4: AI Interpretation** (Generated by phi3:mini)

*Sample output structure:*
- Individual card meanings and relevance to career decision
- How the cards relate to your situation (relocation, career growth, life balance)
- Symbolism analysis (what the images represent about your situation)
- Psychological advice on decision-making and managing life transitions

**Step 5: Response**
User gets instant, personalized guidance combining:
- Tarot wisdom
- Contextual analysis
...
### Adding New Cards to CSV

```csv
card;upright;reversed;symbolism
your-card-name.jpg;Upright meaning text;Reversed meaning text;Symbolism description with details on imagery, colors, and meaning
```

Requirements:
1. Create card image: `your-card-name.jpg` in `images/`
2. Add row to `tarots.csv` with semicolon separator
3. Include complete symbolism description for richer LLM analysis

### Creating Custom Card Spreads

Modify the selectbox in `app.py`:

```python
spread_type = st.selectbox(
    "Choose your spread",
    {
        "3-Card Past-Present-Future": 3,
        "5-Card Relationship": 5,
        "7-Card Chakra": 7,
        "10-Card Celtic Cross": 10
    }
)
```

Then adjust card selection logic accordingly.

### Custom Analysis Instructions

Expand the prompt template to include spread-specific guidance:

```python
prompt_analysis = PromptTemplate.from_template("""
Analyze this {spread_type} reading...
[Your custom instructions]
Cards drawn: {card_details}
Context: {context}
Symbolism: {symbolism}
""")
```
...
- YouTube: Popular tarot education channels
...
**Build by Jagrat Agrawal ✨🔮✨**

---

*Last Updated: February 2026*
*Created with Python, Streamlit, LangChain, and Ollama*
